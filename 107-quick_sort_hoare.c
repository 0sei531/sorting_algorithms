...
{
    quick_sort_extra_hoare(array, 0, size - 1, size);
}

/**
 * quick_sort_extra_hoare - sorts an array of integers in ascending order.
 * @array: the array to be sorted.
...
void quick_sort_extra_hoare(int *array, int lo, int hi, size_t size)
{
    if (lo < hi)
    {
        int pivot = partition_hoare(array, lo, hi, size);
...
/**
 * partition_hoare - partitions an array of integers in ascending order.
 * @array: the array to be sorted.
 * @lo: the beginning of the array
 * @hi: the end of the array
 * @size: the size of the original array
